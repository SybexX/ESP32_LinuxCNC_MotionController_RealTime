...
void IRAM_ATTR commandHandler()
{
    ...
    if (!(fb.control & CTRL_READY))
    {
        ...
        if ((fb.control & CTRL_DIRSETUP) && (fb.control & CTRL_ACCEL) && (fb.control & CTRL_RESTORE) && (fb.control & CTRL_PWMFREQ))
        {
    ...
    if (machineEnabled)
    {
        ...
        */
        ...
        for (uint8_t i = 0; i < configNumSteppers; i++)
        {
            ...
            if (newVel > 0)
            {                                                            // Move is wanted (always a positive value regardless of direction)
...
/*==================================================================*/
/* Used for I2S output boards - DIR/EN pins e.g MKS-DLC32. Not STEP pins (RMT only) */
...
{
    // TODO fix this with new serial console
    if (!consoleLogging)
    {
        return 0;
    }
